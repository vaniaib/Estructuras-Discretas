...
recuperaElemento :: (Ord a) => [a] -> Int -> a
recuperaElemento [] n = error "Indice mayor a la longitud"
recuperaElemento (y:ys) 0 = y
recuperaElemento (x:xs) m = recuperaElemento xs (m-1)
...
extranio :: Int -> [Int]
extranio 1 = [1]
extranio n = if (n `mod` 2 == 0) then n : extranio (n `div` 2)
else n: extranio (3 * n +1)